...
var miolayer = map.getLayer('point');
    ...
    map.fire('click', { lngLat: ll, point:pointsel })
        ...
        map.addLayer({
          ...
          "source": {
              "type": "vector",
              "tiles": ["https://geospatial.jrc.ec.europa.eu/geoserver/gwc/service/wmts?layer=africa_platform:mask_drc&tilematrixset=EPSG:900913&Service=WMTS&Request=GetTile&Version=1.0.0&Format=application/x-protobuf;type=mapbox-vector&TileMatrix=EPSG:900913:{z}&TileCol={x}&TileRow={y}"]
              },
          "source-layer": "mask_drc",
      ...
      map.addLayer({
        ...
        "source": {
            "type": "vector",
            "tiles": ["https://geospatial.jrc.ec.europa.eu/geoserver/gwc/service/wmts?layer=dopa_analyst:cod_bound&tilematrixset=EPSG:900913&Service=WMTS&Request=GetTile&Version=1.0.0&Format=application/x-protobuf;type=mapbox-vector&TileMatrix=EPSG:900913:{z}&TileCol={x}&TileRow={y}"]
            },
        "source-layer": "cod_bound",
 ...
 map.addLayer({
        ...
        "source": {
            "type": "vector",
            "tiles": ["https://geospatial.jrc.ec.europa.eu/geoserver/gwc/service/wmts?layer=dopa_analyst:grid_drc_base_101&tilematrixset=EPSG:900913&Service=WMTS&Request=GetTile&Version=1.0.0&Format=application/x-protobuf;type=mapbox-vector&TileMatrix=EPSG:900913:{z}&TileCol={x}&TileRow={y}"]
            },
        "source-layer": "grid_drc_base_101",
        'paint': {
  ...
  ["in", "adm0_code", ""]
    ...
    var features = map.queryRenderedFeatures({ layers: ["cod_bound"] });
...
layer_country.addEventListener('change', function() {
  ...
  map.setPaintProperty('cod_bound', 'fill-color', ['interpolate',['linear'],['get', layer_country_value],0, '#ffffd4',10, '#ffefb5',50, '#ffde96',100, '#fec46c',150, '#fea73f',250, '#f68c23',350, '#e67217',500, '#d25a0c',750, '#b64708',1000, '#993404']);
});
...
setTimeout(function(){
  ...
  map.on('click', 'cod_bound', function(e) {
    ...
    function toggle(button)
{
  if(document.getElementById("1").value=="OFF"){
   ...
   document.getElementById("1").value="OFF";}
      ...
      var cfeatures = map.queryRenderedFeatures(e.point, { layers: ['cod_bound'] });
      var filter = cfeatures.reduce(function(memo, feature) {
         ...
         map.setFilter("cod_bound", filter);
